...
parser.add_argument("--weightInput", default="0.1", type=str, help="pretrain weight")
...
def main():
    ...
    logger = Logger(t_dir, 'log.txt')
    post_pred = Compose([Activations(softmax=True)])
    post_label = Compose([AsDiscrete(to_onehot=2)])    

    test_dataset = MyDataset(root=args.data_dir, fold=str(args.fold), split='test')
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False)

    print("============= hidden_layer_1 :" + str(hidden_layer_1) + " hidden_layer_2 : " + str(hidden_layer_2) + " batch_size: " + str(args.batch_size) + " fd: " + str(args.fold) +  " dropout " + str(args.dropout))
    ...
    gcnModel.load_state_dict(module_dict, strict=False)     
